...
def main():
	if len(sys.argv) < 4:
        	print
        	print 'fanmode: Fan mode set '
        	print 'ex: fanmode <ip_addr> <bus_num> <0-3>'
        	print 'ex: fanmode 172.30.0.54 1 1'
        	print 'Note: 0=Man, 1=FC_PWM_HWM(default), 2=Auto, 3=MB PWM'
        	print 'try again'
        	exit()
	else:
        	ip = sys.argv[1]
        	bus_num = sys.argv[2]
		fmode = int(sys.argv[3])
	if ( fmode > 3 ) :
        	print 'fan mode is [0-3], 0=Man, 1=FC_PWM_HWM(default), 2=Auto, 3=MB PWM'
        	print 'try again'
        	exit()

	if ( getadt7462PID(ip, bus_num) == 0 ):	 # continue show data
		print linespace 
		changeFanMode(ip, bus_num, fmode)
		time.sleep(3)
		getadt7462speed(ip, bus_num)

		print linespace
		raw_input( "Press any key to continue")
		print "Set it back to default mode, FC_PWM_HWM" 
