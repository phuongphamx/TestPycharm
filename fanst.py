...
def getadt7462pwm(ip, bus):
	pwm = getadt7462byte(ip, bus, hex(0xaa) ) # all channel all same....
	print "fan pwm =", round(int(pwm, 16)/255.0*100,1), '[%]'
...
def main():
	if len(sys.argv) < 3:
        	print
        	print 'fansp: Fan Speed Test '
        	print 'ex: fansp <ip_addr> <bus_num>'
        	print 'ex: fans 172.30.0.54 1'
        	print 'try again'
        	exit()
	else:
        	ip = sys.argv[1]
	...
	if ( getadt7462PID(ip, bus_num) == 0 ):	 # continue show data
		...
		print linespace 
		getadt7462pwm(ip, bus_num)
		print linespace 
		getadt7462temp(ip, bus_num)
		print linespace 
		getadt7462volt(ip, bus_num)
